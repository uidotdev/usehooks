...
// uses polyfill if experimental feature is not available
React.useEffectEvent = React.experimental_useEffectEvent || useEffectEvent
...
): [React.RefObject<Element>, IntersectionObserverEntry | null] {
...
export function useKeyPress(
  ...
  options: {
    event?: string;
    target?: ModifiedWindow | null;
    eventOptions?: AddEventListenerOptions;
  } = {}
): void {
  const { event = "keydown", target = window ?? null, eventOptions } = options;
  const eventOptionsRef = React.useRef<AddEventListenerOptions | undefined>(eventOptions);
...
export function useLongPress(
  ...
  { threshold = 400, onStart, onFinish, onCancel }: CallbackOptions = {}
) {
...
export function useScript(src: string | undefined, options: {
  ...
  const cachedScriptStatuses = React.useRef<Record<string, string>>({})
